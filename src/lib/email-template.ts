...
/**
 * Composizione UNICA del messaggio "Informativa Privacy GDPR firmata".
 * Riusata dal client (sendPrivacyPdf) e dalla server function pubblica di
 * firma via link — §5: nessuna duplicazione del template.
 */
export function buildPrivacyPdfEmailPayload(options: {
  toName: string;
  ragioneSociale: string;
  dataFirma: string | Date;
  /** PDF già codificato base64; se assente la mail viene inviata senza allegato. */
  ...
  const { toName, ragioneSociale, dataFirma, pdfBase64 } = options;
  ...
  const subject = `Informativa Privacy GDPR — ${ragioneSociale}`;
