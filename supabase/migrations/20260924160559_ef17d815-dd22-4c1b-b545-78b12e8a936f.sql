CREATE OR REPLACE FUNCTION public.categoria_scadenza(_stato_contabile text, _data_scadenza date, _data_pagamento_effettiva date, _giorni_ritardo integer, _oggi date)
RETURNS text LANGUAGE sql IMMUTABLE
AS $$
  SELECT CASE
    WHEN _stato_contabile = 'Aperta' AND _data_scadenza IS NOT NULL AND _data_scadenza < _oggi THEN 'scaduto'
    WHEN _data_pagamento_effettiva IS NOT NULL THEN 'pagato'
    WHEN _data_scadenza IS NOT NULL AND _data_scadenza >= _oggi THEN 'a_scadere'
    WHEN _stato_contabile = 'Aperta' AND _data_scadenza IS NULL THEN
      CASE WHEN COALESCE(_giorni_ritardo, 0) > 0 THEN 'scaduto' ELSE 'a_scadere' END
    ELSE 'pagato'
  END
$$;
COMMENT ON FUNCTION public.categoria_scadenza(text, date, date, integer, date) IS 'Fonte unica della categoria scaduto/a_scadere/pagato usata dalle RPC dello scadenziario.';
REVOKE ALL ON FUNCTION public.categoria_scadenza(text, date, date, integer, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.categoria_scadenza(text, date, date, integer, date) TO authenticated, service_role;

DROP FUNCTION public.get_scadenziario_ids(text, uuid, text, text, text, boolean, boolean, text, numeric, boolean, text);
DROP FUNCTION public.get_scadenziario_totali(text, uuid, text, text, text, boolean, boolean, text, numeric, boolean, text);
DROP FUNCTION public.get_scadenziario_lista_paginata(text, uuid, text, text, text, boolean, boolean, text, numeric, boolean, integer, integer, text, text, integer, integer, text);

CREATE FUNCTION public.get_scadenziario_ids(p_search text DEFAULT NULL::text, p_store_id uuid DEFAULT NULL::uuid, p_fascia text DEFAULT 'tutte'::text, p_stato_blocco text DEFAULT 'tutti'::text, p_stato_legale text DEFAULT 'tutti'::text, p_escludi_bonifici boolean DEFAULT true, p_escludi_legale boolean DEFAULT true, p_avvisato text DEFAULT 'tutti'::text, p_importo_min numeric DEFAULT 0, p_mostra_a_credito boolean DEFAULT false, p_agente text DEFAULT NULL::text, p_situazione text DEFAULT 'con_scaduto'::text, p_importo_min_a_scadere numeric DEFAULT 0)
...
BEGIN
  ...
  WITH cls_full AS (
    SELECT s.id, s.cliente_id, s.importo_scadenza, s.codice_pagamento, s.giorni_ritardo,
      ...
      public.categoria_scadenza(s.stato_contabile, s.data_scadenza, s.data_pagamento_effettiva, s.giorni_ritardo, v_today) AS cat
  ...
  agg AS (
    ...
    HAVING (COUNT(*) FILTER (WHERE cat='scaduto') > 0)
        OR (COALESCE(p_situazione,'con_scaduto') <> 'con_scaduto' AND COUNT(*) FILTER (WHERE cat='a_scadere') > 0)
  ),
  agg2 AS (
  ...
  joined AS (
    SELECT cl.id AS cli_id, COALESCE(cl.bloccato,false) AS bloccato,
      ...
      a.tot_s, a.tot_a, a.n_scadute, a.n_a_scadere, a.max_gg,
  ...
  WHERE (CASE WHEN j.tot_s >= 0 THEN j.tot_s >= COALESCE(p_importo_min,0)
    ...
    AND (COALESCE(p_situazione,'con_scaduto') IN ('con_scaduto','tutte')
         OR (p_situazione = 'solo_a_scadere' AND j.n_scadute = 0 AND j.n_a_scadere > 0)
         OR (p_situazione = 'scaduto_e_a_scadere' AND j.n_scadute > 0 AND j.n_a_scadere > 0))
    AND (COALESCE(p_importo_min_a_scadere,0) <= 0 OR j.tot_a >= p_importo_min_a_scadere);
END;
$function$;

CREATE FUNCTION public.get_scadenziario_totali(p_search text DEFAULT NULL::text, p_store_id uuid DEFAULT NULL::uuid, p_fascia text DEFAULT 'tutte'::text, p_stato_blocco text DEFAULT 'tutti'::text, p_stato_legale text DEFAULT 'tutti'::text, p_escludi_bonifici boolean DEFAULT true, p_escludi_legale boolean DEFAULT true, p_avvisato text DEFAULT 'tutti'::text, p_importo_min numeric DEFAULT 0, p_mostra_a_credito boolean DEFAULT false, p_agente text DEFAULT NULL::text, p_situazione text DEFAULT 'con_scaduto'::text, p_importo_min_a_scadere numeric DEFAULT 0)
...
CREATE FUNCTION public.get_scadenziario_lista_paginata(p_search text DEFAULT NULL::text, p_store_id uuid DEFAULT NULL::uuid, p_fascia text DEFAULT 'tutte'::text, p_stato_blocco text DEFAULT 'tutti'::text, p_stato_legale text DEFAULT 'tutti'::text, p_escludi_bonifici boolean DEFAULT true, p_escludi_legale boolean DEFAULT true, p_avvisato text DEFAULT 'tutti'::text, p_importo_min numeric DEFAULT 0, p_mostra_a_credito boolean DEFAULT false, p_anno_corrente integer DEFAULT NULL::integer, p_anno_prec integer DEFAULT NULL::integer, p_sort_by text DEFAULT 'tot_scaduto'::text, p_sort_dir text DEFAULT 'desc'::text, p_page integer DEFAULT 1, p_page_size integer DEFAULT 25, p_agente text DEFAULT NULL::text, p_situazione text DEFAULT 'con_scaduto'::text, p_importo_min_a_scadere numeric DEFAULT 0)
...
BEGIN
  ...
  agg AS (
    ...
    HAVING (COUNT(*) FILTER (WHERE c.cat = 'scaduto') > 0)
        OR (COALESCE(p_situazione,'con_scaduto') <> 'con_scaduto' AND COUNT(*) FILTER (WHERE c.cat = 'a_scadere') > 0)
  ),
  agg2 AS (
  ...
  filtered AS (
    ...
    WHERE (CASE WHEN j.tot_scaduto >= 0 THEN j.tot_scaduto >= COALESCE(p_importo_min, 0)
      ...
      AND (COALESCE(p_situazione,'con_scaduto') IN ('con_scaduto','tutte')
           OR (p_situazione = 'solo_a_scadere' AND j.n_scadute = 0 AND j.n_a_scadere > 0)
           OR (p_situazione = 'scaduto_e_a_scadere' AND j.n_scadute > 0 AND j.n_a_scadere > 0))
      AND (COALESCE(p_importo_min_a_scadere,0) <= 0 OR j.tot_a_scadere >= p_importo_min_a_scadere)
  ),
  ...
  ORDER BY
    ...
    CASE WHEN p_sort_by = 'codice_gestionale' AND p_sort_dir = 'asc'  THEN f.codice_gestionale END ASC NULLS LAST,
    CASE WHEN p_sort_by = 'codice_gestionale' AND p_sort_dir = 'desc' THEN f.codice_gestionale END DESC NULLS LAST,
    CASE WHEN p_sort_by = 'store_nome' AND p_sort_dir = 'asc'  THEN f.store_nome END ASC NULLS LAST,
    CASE WHEN p_sort_by = 'store_nome' AND p_sort_dir = 'desc' THEN f.store_nome END DESC NULLS LAST,
    CASE WHEN p_sort_by = 'fatturato_cur' AND p_sort_dir = 'asc'  THEN f.fatturato_cur END ASC NULLS LAST,
    CASE WHEN p_sort_by = 'fatturato_cur' AND p_sort_dir = 'desc' THEN f.fatturato_cur END DESC NULLS LAST,
    CASE WHEN p_sort_by = 'fatturato_prec' AND p_sort_dir = 'asc'  THEN f.fatturato_prec END ASC NULLS LAST,
    CASE WHEN p_sort_by = 'fatturato_prec' AND p_sort_dir = 'desc' THEN f.fatturato_prec END DESC NULLS LAST,
    CASE WHEN p_sort_by = 'n_scadute' AND p_sort_dir = 'asc'  THEN f.n_scadute END ASC NULLS LAST,
    CASE WHEN p_sort_by = 'n_scadute' AND p_sort_dir = 'desc' THEN f.n_scadute END DESC NULLS LAST,
    CASE WHEN p_sort_by = 'n_a_scadere' AND p_sort_dir = 'asc'  THEN f.n_a_scadere END ASC NULLS LAST,
    CASE WHEN p_sort_by = 'n_a_scadere' AND p_sort_dir = 'desc' THEN f.n_a_scadere END DESC NULLS LAST,
    CASE WHEN p_sort_by = 'prossima_scadenza' AND p_sort_dir = 'asc'  THEN f.prossima_scadenza END ASC NULLS LAST,
    CASE WHEN p_sort_by = 'prossima_scadenza' AND p_sort_dir = 'desc' THEN f.prossima_scadenza END DESC NULLS LAST,
    CASE WHEN p_sort_by = 'bloccato' AND p_sort_dir = 'asc'  THEN f.bloccato END ASC NULLS LAST,
    CASE WHEN p_sort_by = 'bloccato' AND p_sort_dir = 'desc' THEN f.bloccato END DESC NULLS LAST,
    f.ragione_sociale ASC
  OFFSET v_offset LIMIT v_limit;
END;
$function$;

REVOKE ALL ON FUNCTION public.get_scadenziario_ids(text, uuid, text, text, text, boolean, boolean, text, numeric, boolean, text, text, numeric) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_scadenziario_totali(text, uuid, text, text, text, boolean, boolean, text, numeric, boolean, text, text, numeric) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_scadenziario_lista_paginata(text, uuid, text, text, text, boolean, boolean, text, numeric, boolean, integer, integer, text, text, integer, integer, text, text, numeric) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_scadenziario_ids(text, uuid, text, text, text, boolean, boolean, text, numeric, boolean, text, text, numeric) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_scadenziario_totali(text, uuid, text, text, text, boolean, boolean, text, numeric, boolean, text, text, numeric) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_scadenziario_lista_paginata(text, uuid, text, text, text, boolean, boolean, text, numeric, boolean, integer, integer, text, text, integer, integer, text, text, numeric) TO authenticated, service_role;