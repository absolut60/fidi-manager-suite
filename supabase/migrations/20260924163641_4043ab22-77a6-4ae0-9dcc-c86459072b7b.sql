...
BEGIN
  ...
  joined AS (
    SELECT
      ...
      COALESCE(ag2.n_scadute, 0) AS n_scadute, COALESCE(ag2.tot_scaduto, 0) AS tot_scaduto,
      COALESCE(ag2.n_a_scadere, 0) AS n_a_scadere, COALESCE(ag2.tot_a_scadere, 0) AS tot_a_scadere,
      ag2.prossima_scadenza, COALESCE(ag2.max_gg, 0) AS max_gg_ritardo, COALESCE(ag2.scadute_ids, ARRAY[]::uuid[]) AS scadute_ids,
      CASE WHEN COALESCE(ag2.max_gg, 0) <= 0 THEN NULL WHEN ag2.max_gg <= 30 THEN '0_30' WHEN ag2.max_gg <= 60 THEN '31_60' ELSE 'oltre_60' END AS fascia,
    ...
    LEFT JOIN agg2 ag2 ON ag2.cli_id = cl.id
    ...
    WHERE (ag2.cli_id IS NOT NULL OR p_situazione = 'portafoglio')
      AND public.user_can_access_cliente(cl.id, cl.store_id, cl.codice_agente)
  ...
  filtered AS (
    ...
    WHERE (CASE WHEN j.tot_scaduto >= 0 THEN j.tot_scaduto >= COALESCE(p_importo_min, 0)
      ...
      AND (COALESCE(p_situazione,'con_scaduto') IN ('con_scaduto','tutte','portafoglio')
  ...
  ORDER BY
    ...
    CASE WHEN p_sort_by = 'codice_gestionale' AND p_sort_dir = 'asc'  THEN lpad(COALESCE(f.codice_gestionale,''), 20, '0') END ASC NULLS LAST,
    CASE WHEN p_sort_by = 'codice_gestionale' AND p_sort_dir = 'desc' THEN lpad(COALESCE(f.codice_gestionale,''), 20, '0') END DESC NULLS LAST,
...
BEGIN
  ...
  joined AS (
    SELECT cl.id, COALESCE(cl.bloccato,false) AS bloccato,
      COALESCE(cl.in_gestione_legale,false) AS in_gestione_legale,
      COALESCE(a.tot_s, 0) AS tot_s, COALESCE(a.tot_a, 0) AS tot_a, COALESCE(a.n_scadute, 0) AS n_scadute,
      COALESCE(a.n_a_scadere, 0) AS n_a_scadere, COALESCE(a.max_gg, 0) AS max_gg,
      COALESCE(av.n_az, 0) AS avvisato_n,
      CASE WHEN COALESCE(a.max_gg, 0) <= 0 THEN NULL WHEN a.max_gg <= 30 THEN '0_30' WHEN a.max_gg <= 60 THEN '31_60' ELSE 'oltre_60' END AS fascia
    FROM public.clienti cl
    LEFT JOIN agg2 a ON a.cliente_id = cl.id
    LEFT JOIN avv av ON av.cliente_id = cl.id
    WHERE (a.cliente_id IS NOT NULL OR p_situazione = 'portafoglio')
      AND public.user_can_access_cliente(cl.id, cl.store_id, cl.codice_agente)
      AND (p_store_id IS NULL OR cl.store_id = p_store_id)
      AND (p_stato_blocco = 'tutti' OR (p_stato_blocco = 'bloccati' AND COALESCE(cl.bloccato,false) = true) OR (p_stato_blocco = 'non_bloccati' AND COALESCE(cl.bloccato,false) = false))
      AND (p_stato_legale = 'tutti' OR (p_stato_legale = 'in_legale' AND COALESCE(cl.in_gestione_legale,false) = true) OR (p_stato_legale = 'non_in_legale' AND COALESCE(cl.in_gestione_legale,false) = false))
  ...
  leg AS (
    SELECT COUNT(DISTINCT cl.id)::int AS n FROM public.clienti cl
    LEFT JOIN agg2 a ON a.cliente_id = cl.id
    WHERE (a.cliente_id IS NOT NULL OR p_situazione = 'portafoglio')
      AND COALESCE(cl.in_gestione_legale,false) = true
      AND public.user_can_access_cliente(cl.id, cl.store_id, cl.codice_agente)
      AND (p_store_id IS NULL OR cl.store_id = p_store_id)
      AND (p_agente IS NULL OR (p_agente = '__none__' AND cl.codice_agente IS NULL) OR cl.codice_agente = p_agente)
  )
...
CREATE OR REPLACE FUNCTION public.get_scadenziario_ids(p_search text DEFAULT NULL::text, p_store_id uuid DEFAULT NULL::uuid, p_fascia text DEFAULT 'tutte'::text, p_stato_blocco text DEFAULT 'tutti'::text, p_stato_legale text DEFAULT 'tutti'::text, p_escludi_bonifici boolean DEFAULT true, p_escludi_legale boolean DEFAULT true, p_avvisato text DEFAULT 'tutti'::text, p_importo_min numeric DEFAULT 0, p_mostra_a_credito boolean DEFAULT false, p_agente text DEFAULT NULL::text, p_situazione text DEFAULT 'con_scaduto'::text, p_importo_min_a_scadere numeric DEFAULT 0)
 RETURNS TABLE(cliente_id uuid)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  joined AS (
    SELECT cl.id AS cli_id, COALESCE(cl.bloccato,false) AS bloccato,
      ...
      CASE WHEN COALESCE(a.max_gg, 0) <= 0 THEN NULL WHEN a.max_gg <= 30 THEN '0_30' WHEN a.max_gg <= 60 THEN '31_60' ELSE 'oltre_60' END AS fascia
    FROM public.clienti cl
    LEFT JOIN agg2 a ON a.cli_id = cl.id
    LEFT JOIN avv av ON av.cli_id = cl.id
    WHERE (a.cli_id IS NOT NULL OR p_situazione = 'portafoglio')
      AND public.user_can_access_cliente(cl.id, cl.store_id, cl.codice_agente)
      AND (p_store_id IS NULL OR cl.store_id = p_store_id)
      AND (p_stato_blocco = 'tutti' OR (p_stato_blocco = 'bloccati' AND COALESCE(cl.bloccato,false) = true) OR (p_stato_blocco = 'non_bloccati' AND COALESCE(cl.bloccato,false) = false))
      AND (p_stato_legale = 'tutti' OR (p_stato_legale = 'in_legale' AND COALESCE(cl.in_gestione_legale,false) = true) OR (p_stato_legale = 'non_in_legale' AND COALESCE(cl.in_gestione_legale,false) = false))
