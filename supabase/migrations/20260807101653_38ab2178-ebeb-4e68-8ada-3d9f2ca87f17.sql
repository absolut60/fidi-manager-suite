...
RETURNS TABLE(
  ...
  fido_concesso_clienti bigint,
  fido_concesso_piccoli_n bigint,
  fido_concesso_piccoli_eur numeric,
  fido_proposto_eur numeric,
  fido_proposto_clienti bigint,
  fido_proposto_piccoli_n bigint,
  fido_proposto_piccoli_eur numeric,
  da_verificare_n bigint,
  oltre_fido_n bigint,
  oltre_fido_eur numeric,
  bloccati_n bigint,
  bloccati_eur numeric,
  insoluti_n bigint,
  insoluti_eur numeric,
  insoluti_non_bloccati_n bigint,
  scaduto_eur numeric,
  scaduto_over60_eur numeric,
  aggiornato_al text
)
LANGUAGE sql
...
AS $function$
  ...
  sc AS (
    SELECT
      COALESCE(SUM(s.importo_residuo), 0) AS tot,
      COALESCE(SUM(s.importo_residuo) FILTER (WHERE s.data_scadenza < CURRENT_DATE - 60), 0) AS over60
    FROM public.scadenze s
    JOIN cl ON cl.id = s.cliente_id
    WHERE s.data_pagamento_effettiva IS NULL
      AND COALESCE(s.importo_residuo, 0) > 0
  ...
  SELECT
    ...
    (SELECT COALESCE(SUM(COALESCE(totale_rischio, 0) + COALESCE(doc_da_evadere, 0) - COALESCE(fido_gestionale, 0)), 0)
       FROM cl WHERE COALESCE(attivo, true)
        AND COALESCE(totale_rischio, 0) + COALESCE(doc_da_evadere, 0) > COALESCE(fido_gestionale, 0)),
    (SELECT COUNT(*) FROM cl WHERE COALESCE(attivo, true) AND bloccato AND COALESCE(fido_gestionale, 0) > 0),
    (SELECT COALESCE(SUM(fido_gestionale), 0) FROM cl WHERE COALESCE(attivo, true) AND bloccato AND COALESCE(fido_gestionale, 0) > 0),
