...
DECLARE
  ...
  v_eur_totale numeric := 0;
  v_ritardo numeric := 0;
  v_eur_txt text;
  v_eur_totale_txt text;
  v_stadio text;
  v_motivo text;
BEGIN
  ...
  -- Scaduto GRAVE: solo fasce oltre 60 giorni (invariato)
  ...
  -- Scaduto TOTALE: tutte le fasce scadute (0-30, 30-60, 60-90, 90-120, oltre 120)
  SELECT COALESCE(SUM(s.importo_scadenza), 0)
    INTO v_eur_totale
  FROM public.scadenze s
  WHERE s.cliente_id = p_cliente_id
    AND s.stato_contabile = 'Aperta'
  ...
  v_eur_txt := translate(to_char(v_eur_grave, 'FM999G999G990D00'), ',.', '.,');
  v_eur_totale_txt := translate(to_char(v_eur_totale, 'FM999G999G990D00'), ',.', '.,');
  ...
  ELSIF v_eur_totale > 10000 THEN
    v_stadio := 'rosso'; v_motivo := 'Scaduto totale ' || v_eur_totale_txt || ' €';
  ELSIF v_n_grave > 0 THEN
    v_stadio := 'arancione'; v_motivo := 'Scaduto ' || v_eur_txt || ' € fermo oltre 60 giorni';
  ELSIF v_eur_totale >= 1000 AND v_eur_totale <= 10000 THEN
    v_stadio := 'arancione'; v_motivo := 'Scaduto ' || v_eur_totale_txt || ' €';
  ELSIF v_eur_totale > 0 AND v_eur_totale < 1000 THEN
    v_stadio := 'giallo'; v_motivo := 'Scaduto ' || v_eur_totale_txt || ' €';
  ELSIF v_ritardo > 15 THEN
    v_stadio := 'giallo'; v_motivo := 'Ritardo medio ' || ROUND(v_ritardo)::text || ' giorni sui pagamenti';
  ELSE
    v_stadio := 'verde';
