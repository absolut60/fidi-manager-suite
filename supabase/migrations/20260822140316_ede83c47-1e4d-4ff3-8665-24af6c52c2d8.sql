CREATE OR REPLACE FUNCTION public.get_dashboard_fidi_aggregati()
 RETURNS TABLE(tipo text, chiave text, etichetta text, ordine integer, n_clienti bigint, n_clienti_con_fido bigint, fido_concesso_eur numeric, fido_proposto_eur numeric)
 LANGUAGE sql
...
AS $function$
  ...
  ft AS (SELECT cliente_id, COALESCE(fido_proposto, 0) AS proposto FROM public.fido_teorico_cliente),
  base AS (
