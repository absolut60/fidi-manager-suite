...
-- ============================================================
-- Regola SCADUTO unificata: anticipi sottratti + clamp per cliente
-- Fonte di verita' TS: src/lib/scadenze.ts (isAnticipo / sommaScadutoCliente)
-- Anticipo = numero_documento ILIKE '%ANTICIPO%'
-- ============================================================
...
AS $function$
  -- Regola anticipi+clamp allineata a src/lib/scadenze.ts (fonte di verita' TS).
  WITH cls AS (
    SELECT
  ...
  SELECT cliente_id,
    GREATEST(
      COALESCE(SUM(CASE WHEN is_anticipo THEN -importo_scadenza ELSE importo_scadenza END)
               FILTER (WHERE categoria='scaduto'), 0),
      0
    ),
...
AS $function$
  -- Regola anticipi+clamp allineata a src/lib/scadenze.ts (fonte di verita' TS).
  -- Fonte di verita' validazione email: src/lib/email-validazione.ts
  WITH cls AS (
...
AS $function$
  ...
  scad AS (
    SELECT s.cliente_id,
      GREATEST(
        SUM(CASE WHEN s.numero_documento ILIKE '%ANTICIPO%' THEN -s.importo_scadenza ELSE s.importo_scadenza END),
        0
      ) AS totale_scaduto
    FROM public.scadenze s
...
BEGIN
  ...
  agg AS (
    SELECT
      ...
      GREATEST(
        COALESCE(SUM(CASE WHEN c.is_anticipo THEN -c.importo_scadenza ELSE c.importo_scadenza END)
                 FILTER (WHERE c.cat = 'scaduto'), 0),
        0
      )::numeric                                                                            AS tot_scaduto,
