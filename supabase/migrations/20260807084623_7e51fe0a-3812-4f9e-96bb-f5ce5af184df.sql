...
VALUES ('fido_teorico_escludi_sede', 'false', 'Se attivo, i clienti della sede di Cinisello Balsamo mantengono il fido attuale e non ricevono una proposta')
...
AS $function$
  ...
  base AS (
    SELECT
      ...
      b.insoluti, b.gg_oltre, cfg.usa_coef, cfg.escludi_sede,
  ...
  nota AS (
    SELECT r.*,
      (r.fa > 0 AND r.fp < r.fa * 0.5) AS riduzione_forte,
      (r.fa > 0 AND r.fp > r.fa * 2) AS aumento_forte,
      -- pezzo 1: variazione
      CASE
      ...
      END
      ||
      -- pezzo 2: regola speciale o coefficiente
      CASE r.regola
