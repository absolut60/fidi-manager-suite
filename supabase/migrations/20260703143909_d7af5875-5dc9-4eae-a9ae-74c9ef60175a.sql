...
GRANT EXECUTE ON FUNCTION public.get_incassi_periodo(date, date, text, text[]) TO authenticated, service_role;


DROP FUNCTION IF EXISTS public.get_incassi_periodo_dettaglio(date, date, uuid);
DROP FUNCTION IF EXISTS public.get_incassi_periodo_dettaglio(date, date, uuid, text[]);
...
AS $$
  ...
  WHERE s.cliente_id = _cliente_id
    ...
    AND COALESCE(s.importo_scadenza, 0) <> 0
    AND (
      _metodi IS NULL OR cardinality(_metodi) = 0
      OR (
        CASE
          WHEN upper(COALESCE(s.codice_pagamento,'')) LIKE 'RB%' THEN 'RiBa'
          WHEN upper(COALESCE(s.codice_pagamento,'')) LIKE 'BO%' THEN 'Bonifico'
...
GRANT EXECUTE ON FUNCTION public.get_incassi_periodo_dettaglio(date, date, uuid, text[]) TO authenticated, service_role;
