...
-- 2) get_recupero_clienti_aggregato (cambia solo CTE scad; resta tutto il resto)
...
RETURNS TABLE(
  cliente_id uuid, ragione_sociale text, store_id uuid, store_nome text,
  totale_scaduto numeric, azioni_totali integer, azioni_aperte integer,
  prossima_tipo text, prossima_data timestamptz, ultima_fatta_tipo text, ultima_fatta_data timestamptz,
  ha_promessa boolean, data_promessa timestamptz, in_ritardo boolean,
  stadio_sollecito smallint, stadio_data timestamptz, stadio_giorni integer)
LANGUAGE sql STABLE
SET search_path TO 'public'
AS $$
  WITH az AS (
  ...
  scad AS (
    ...
    WHERE s.stato_contabile = 'Aperta'
      AND s.data_pagamento_effettiva IS NULL
...
-- 3) get_clienti_senza_email_con_scadenze
...
-- 4) get_coerenza_escalation (cambia solo il filtro "aperte/scadute")
CREATE OR REPLACE FUNCTION public.get_coerenza_escalation(_cliente_ids uuid[], _livello_precedente smallint)
RETURNS TABLE(cliente_id uuid, scadenze_aperte_correnti uuid[], scadenze_precedente uuid[], ha_azione_precedente boolean, scaduto_cambiato boolean, data_azione_precedente timestamptz)
LANGUAGE sql STABLE
SET search_path TO 'public'
AS $$
...
-- 5) get_clienti_avvisati (filtro aperte = stato_contabile + no data pagamento)
CREATE OR REPLACE FUNCTION public.get_clienti_avvisati()
RETURNS TABLE(cliente_id uuid, n_azioni integer, ha_email boolean, ultima_tipo text, ultima_data timestamptz)
LANGUAGE sql STABLE
SET search_path TO 'public'
AS $$
  WITH aperte AS (
    ...
    FROM public.scadenze s
    WHERE s.stato_contabile = 'Aperta' AND s.data_pagamento_effettiva IS NULL
  ),
  azioni_linked AS (
...
-- 6) get_promemoria_clienti_aggregato: era basato su 'scader' in tempi_scadenza.
-- Nuovo: Aperta + data_scadenza nel futuro all'interno dei mesi richiesti.
CREATE OR REPLACE FUNCTION public.get_promemoria_clienti_aggregato(
  _mesi text[], _store_id uuid DEFAULT NULL, _search text DEFAULT NULL,
  _importo_min numeric DEFAULT NULL, _escludi_legale boolean DEFAULT true, _escludi_bloccati boolean DEFAULT false)
...
-- 7) genera_snapshot: stessa regola
