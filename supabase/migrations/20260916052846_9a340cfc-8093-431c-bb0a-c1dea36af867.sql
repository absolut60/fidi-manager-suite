...
BEGIN
  ...
  IF p.contatto_id IS NOT NULL THEN
    SELECT * INTO ct FROM public.contatti WHERE id = p.contatto_id;
  END IF;
  ...
  SELECT f.tipo, f.id, f.etichetta,
         (SELECT array_agg(DISTINCT m) FROM unnest(f.motivi) AS u(m)) AS motivi,
         bool_or(f.forte) AS forte
  FROM filtrati f
  GROUP BY f.tipo, f.id, f.etichetta
  ORDER BY bool_or(f.forte) DESC,
           cardinality((SELECT array_agg(DISTINCT m) FROM unnest(f.motivi) AS u(m))) DESC;
END;
$$;

REVOKE EXECUTE ON FUNCTION public.cerca_candidati_riconciliazione(uuid) FROM anon, public;
GRANT EXECUTE ON FUNCTION public.cerca_candidati_riconciliazione(uuid) TO authenticated, service_role;