...
BEGIN
  ...
  UPDATE public.messaggi_whatsapp AS mw
  SET
    mw.stato = _stato_interno,
    mw.consegnato_at = CASE
      WHEN _stato_interno = 'consegnato' AND mw.consegnato_at IS NULL THEN now()
      WHEN _stato_interno = 'letto'      AND mw.consegnato_at IS NULL THEN now()
      ELSE mw.consegnato_at
    END,
    mw.letto_at = CASE
      WHEN _stato_interno = 'letto' AND mw.letto_at IS NULL THEN now()
      ELSE mw.letto_at
    END,
    mw.errore = CASE
  ...
  IF _mw.stato = 'inviato' AND _stato_interno = 'fallito' THEN
    UPDATE public.campagne_whatsapp AS cw
    SET
      cw.invii_falliti = cw.invii_falliti + 1,
      cw.invii_ok      = GREATEST(cw.invii_ok - 1, 0)
    WHERE cw.id = _mw.campagna_id;
  END IF;
