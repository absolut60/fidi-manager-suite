-- NOTA TECNICA (limite dichiarato):
-- clienti.doc_da_fatturare e' una FOTOGRAFIA DEL MOMENTO, non una media.
-- Di conseguenza il fido proposto oscillera' con il magazzino in transito:
-- un cliente con molte consegne non ancora fatturate risultera' avere bisogno
-- di piu' fido di quanto gli serva a regime. E' una scelta prudenziale voluta,
-- ma va ricordata quando si interpretano le variazioni fra un ricalcolo e l'altro.

INSERT INTO public.configurazioni (chiave, valore, descrizione)
VALUES ('fido_teorico_includi_ddt', 'true',
        'Somma al fido base i documenti consegnati e non ancora fatturati (merce gia uscita che diventera esposizione)')
...
AS $function$
  ...
  base AS (
    SELECT
      ...
      b.insoluti, b.gg_oltre, cfg.usa_coef, cfg.escludi_sede, cfg.soglia, cfg.includi_ddt,
  ...
  ris AS (
    SELECT f.*,
      -- DDT sommati solo nei rami fascia_500 / fascia_5000 e solo se positivi
...
GRANT EXECUTE ON FUNCTION public.get_fido_teorico(uuid[], boolean) TO authenticated;